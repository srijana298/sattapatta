...
const DashboardOverview: React.FC = () => {
  ...
  const upcomingAppointments = bookings.filter(booking => 
    new Date(booking.startTime) >= now && new Date(booking.startTime) <= in48Hours
  );
                ...
                <div key={appointment.id} className="p-6 hover:bg-gray-50">
                  <div className="flex items-start justify-between">
                    <div className="flex items-start space-x-4">
                      ...
                      />
                      <div>
                        <h3 className="text-sm font-medium text-gray-800">{appointment.studentName}</h3>
                    ...
                    <div className="flex space-x-2">
                      <button className="bg-orange-100 text-orange-600 hover:bg-orange-200 px-3 py-1.5 
                                       rounded-md text-xs font-medium transition-colors duration-150 ease-in-out">
                        Join Call
                      </button>
                      <button className="bg-gray-100 text-gray-600 hover:bg-gray-200 px-3 py-1.5 
                                       rounded-md text-xs font-medium transition-colors duration-150 ease-in-out">
                        Reschedule
                      </button>
                    </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div className="divide-y divide-gray-200">
          {messages.slice(0, 3).map((message) => (
            <div key={message.id} className={`p-4 hover:bg-gray-50 ${!message.read ? 'bg-blue-50' : ''}`}>
